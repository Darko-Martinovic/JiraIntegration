...
public class JiraReportingService : BaseJiraHttpService, IJiraReportingService
{
    ...
    public async Task<SprintReport> GenerateSprintReportAsync(string sprintId)
    {
        try
        {
            _logger.LogInformation("Generating sprint report for sprint: {SprintId}", sprintId);

            // Get sprint issues using JQL
            var jql = $"sprint = {sprintId}";
            var response = await GetAsync<JiraSearchResponse>($"/rest/api/3/search?jql={Uri.EscapeDataString(jql)}&maxResults=1000");
            ...
            var completedIssues = response.Issues.Where(i =>
                i.Fields.Status.Name.Equals("Done", StringComparison.OrdinalIgnoreCase) ||
                i.Fields.Status.Name.Equals("Resolved", StringComparison.OrdinalIgnoreCase)).ToList();
            ...
            _logger.LogInformation("Sprint report generated for {SprintId}. Completed: {Completed}, Incomplete: {Incomplete}",
                sprintId, completedIssues.Count, incompleteIssues.Count);
    ...
    public async Task<TeamDashboard> GenerateTeamDashboardAsync(string projectKey)
    {
        try
        {
            _logger.LogInformation("Generating team dashboard for project: {ProjectKey}", projectKey);
            ...
            var workloadsByUser = response.Issues
                ...
                .Select(g => new TeamMemberWorkload
                {
                    UserName = g.Key,
                    OpenTickets = g.Count(i => !i.Fields.Status.Name.Equals("Done", StringComparison.OrdinalIgnoreCase)),
                    InProgressTickets = g.Count(i => i.Fields.Status.Name.Contains("Progress", StringComparison.OrdinalIgnoreCase)),
                    CompletedTickets = g.Count(i => i.Fields.Status.Name.Equals("Done", StringComparison.OrdinalIgnoreCase)),
            ...
            _logger.LogInformation("Team dashboard generated for project: {ProjectKey}", projectKey);
            return dashboard;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error generating team dashboard for project: {ProjectKey}", projectKey);
            throw;
        }
    }
    ...
    public async Task<ExecutiveSummary> GenerateExecutiveSummaryAsync(string projectKey)
    {
        try
        {
            _logger.LogInformation("Generating executive summary for project: {ProjectKey}", projectKey);
            ...
            var completedIssues = response.Issues.Count(i =>
                i.Fields.Status.Name.Equals("Done", StringComparison.OrdinalIgnoreCase) ||
                i.Fields.Status.Name.Equals("Resolved", StringComparison.OrdinalIgnoreCase));
            var inProgressIssues = response.Issues.Count(i =>
                i.Fields.Status.Name.Contains("Progress", StringComparison.OrdinalIgnoreCase));
            var openIssues = totalIssues - completedIssues - inProgressIssues;

            var completionPercentage = totalIssues > 0 ? (double)completedIssues / totalIssues * 100 : 0;
            ...
            var summary = new ExecutiveSummary
            {
                ...
                {
                    openIssues > inProgressIssues ? "High number of open tickets" : "Manageable backlog",
                    completionPercentage < 50 ? "Low completion rate needs attention" : "Good progress rate"
                }
            };

            _logger.LogInformation("Executive summary generated for project: {ProjectKey}. Completion: {Completion}%",
                projectKey, completionPercentage);

            return summary;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error generating executive summary for project: {ProjectKey}", projectKey);
            throw;
        }
    }
    ...
    public List<ReportType> GetAvailableReportTypes()
    {
        ...
        {
            new() { Id = "sprint", Name = "Sprint Report", Description = "Sprint burndown and velocity tracking", RequiredParameters = new List<string> { "sprintId" } },
            new() { Id = "team", Name = "Team Dashboard", Description = "Team workload and distribution", RequiredParameters = new List<string> { "projectKey" } },
            new() { Id = "executive", Name = "Executive Summary", Description = "High-level project status", RequiredParameters = new List<string> { "projectKey" } },
            new() { Id = "velocity", Name = "Velocity Chart", Description = "Team velocity over time", RequiredParameters = new List<string> { "projectKey", "sprints" } },
            new() { Id = "burndown", Name = "Burndown Chart", Description = "Sprint or release burndown", RequiredParameters = new List<string> { "sprintId" } }
        };
    }
}
